...
async def get_all_chat_requests_for_user(db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    user_id = current_user.id
    return ChatService(db).get_all_chat_requests_for_user(user_id)
